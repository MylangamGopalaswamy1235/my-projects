...
if RetinaFace is not None:
    try:
        faces = RetinaFace.detect_faces(img_path)  # or use the OpenCV frame
        if isinstance(faces, dict) and len(faces) > 0:
            print(f"✅ Face detected: {len(faces)} face(s)")
            first_face = list(faces.values())[0]
            facial_area = first_face["facial_area"]  # [x1, y1, x2, y2]
        ...
        print("⚠️ RetinaFace detection failed:", e)
        face_crop = frame
else:
    print("⚠️ RetinaFace not available, using full frame.")
    face_crop = frame
